...
class Gr00tN1d7Processor(BaseProcessor):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str | Path, **kwargs):
        ...
        # On Windows, Path("org/model") becomes "org\model" and breaks HF hub IDs.
        repo_or_path = str(pretrained_model_name_or_path)
        is_local = os.path.isdir(repo_or_path)
        if is_local:
            pretrained_model_name_or_path = Path(repo_or_path)
            ...
            hub_id = repo_or_path.replace("\\", "/")
            config_file = Path(cached_file(hub_id, "processor_config.json", **hub_kwargs))
            statistics_file = Path(cached_file(hub_id, "statistics.json", **hub_kwargs))
            embodiment_id_file = Path(cached_file(hub_id, "embodiment_id.json", **hub_kwargs))
