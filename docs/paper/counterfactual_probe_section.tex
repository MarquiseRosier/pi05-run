...
% \RESULT{...} marks a value still to be filled; the artefact that fills each table is
% named in a comment above it. Notation follows the transcoder section of the main paper.
...
Positive control, prompt swap & \RESULT{val} & $0$ & $0$ & $0.729$ \\
Target object                 & $10.11$ & \RESULT{val} & \RESULT{val} & $0.538$ \\
Placebo object                & $1.42$  & \RESULT{val} & \RESULT{val} & $0.032$ \\
\hline
Ratio, target over placebo    & $7.13$ & \RESULT{val} & \RESULT{val} & $17.0$ \\
\hline
\multicolumn{5}{l}{Raw $\mathrm{Sel}$ by cell: mean $8.56\pm5.95$, range $2.92$ to $15.7$, 95\% CI $[4.00,\,11.4]$; by cluster: $3.02$ to $14.4$} \\
\multicolumn{5}{l}{$\mathrm{Sel}_{\mathrm{adj}}$, Eq.~\ref{eq:sel-px}, $S=\lVert\Delta I\rVert_2$: pooled \RESULT{val}; cells min \RESULT{val}, max \RESULT{val}; 95\% CI [\RESULT{lo}, \RESULT{hi}]; $S=|\mathcal{P}|$: pooled \RESULT{val}} \\
\multicolumn{5}{l}{Robustness: per-layer geometric mean $7.33$ [$5.32$, $11.3$], $18$ of $18$ layers above 1; relative-$\ell_2$ ratio \RESULT{val}} \\
\multicolumn{5}{l}{Dose: target $D$ $9.49\rightarrow10.7$, placebo $1.25\rightarrow1.59$, both monotone; $\eta(\mathrm{target})=\RESULT{val}$, $\eta(\mathrm{placebo})=\RESULT{val}$} \\
\hline
\end{tabular}
\end{center}
\end{table}
...
cells, with a minimum of $2.9$; it is heterogeneous across states, near $3$ at
the initial state and near $14$ five plan actions later, when the arm has
approached the bowl and the wrist camera carries most of the change. The
footprint-adjusted figure is \RESULT{val}. Selectivity holds in all $18$ layers
...
must re-plan. The response is sublinear in dose for the target, near $90\%$ of
its full-dose value at half dose, which is consistent with a categorical rather
than graded reading of the colour.
...
\paragraph{H1.} \RESULT{supported}. The rule is: invalid if the null floor
exceeds $5\%$ of the placebo response; supported if the pooled
...
otherwise. The floor is zero and the unadjusted ratio exceeds one in every cell
with a minimum of $2.9$; the footprint-adjusted figure on which the rule is
stated is \RESULT{val}. The conclusion is robust across the three summaries.
...
completes. The pipeline is probe, report, discovery, trace, validate; the
