...
try:
    ...
    if os.environ.get("GROOT_ANALYSIS_NO_UV_REEXEC") != "1" and shutil.which("uv"):
        env = os.environ.copy()
        env["GROOT_ANALYSIS_NO_UV_REEXEC"] = "1"
        os.execvpe("uv", ["uv", "run", "python", __file__, *sys.argv[1:]], env)
    raise


ROOT = Path("outputs/eval/groot_libero")
CANVAS_W = 1920
CANVAS_H = 1080
PANEL_W = CANVAS_W // 2
PANEL_H = CANVAS_H // 2
...
def find_task_video(run_dir: Path, task_id: int, episode: int) -> Path:
    ...
    if matches:
        return matches[0]
    matches = sorted((run_dir / "videos").glob(f"*/eval_episode_{episode}.mp4"))
    if matches:
        return matches[0]
    matches = sorted((run_dir / "videos").glob("**/*.mp4"))
    if matches:
        return matches[0]
...
def render_action(capture: dict[str, Any], chunk: int, phase: int, n_action_steps: int, scales: dict[str, float]) -> np.ndarray:
    ...
    panel = np.zeros((PANEL_H, PANEL_W, 3), dtype=np.uint8)
    panel[:] = PANEL_BG
    draw_header(panel, "Signed Action Chunk", f"chunk {chunk} | executes first {n_action_steps} actions then replans")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a diagnostic activation video for GR00T LIBERO runs.")
    ...
    if not metrics and eval_info.get("overall_success_rate") is not None:
        metrics = {"pc_success": round(float(eval_info["overall_success_rate"]) * 100.0, 2)}
