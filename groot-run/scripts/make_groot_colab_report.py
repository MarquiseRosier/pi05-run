...
try:
    ...
    if os.environ.get("GROOT_REPORT_NO_UV_REEXEC") != "1" and shutil.which("uv"):
        env = os.environ.copy()
        env["GROOT_REPORT_NO_UV_REEXEC"] = "1"
        os.execvpe("uv", ["uv", "run", "python", __file__, *sys.argv[1:]], env)
    raise

sys.path.insert(0, str(Path(__file__).resolve().parent))
from make_groot_analysis_video import (  # noqa: E402
    BG,
    GRID,
    MUTED,
    PANEL_BG,
...
def make_chunk_matrix(
    *,
    ...
    for chunk in chunks:
        ...
        if task:
            break
    put_text(canvas, f"GR00T LIBERO Chunk Matrix | task_id={task_id} episode={episode}", (18, 32), scale=0.76, color=TEXT, thickness=2)
    put_text(
        canvas,
...
<head>
  <meta charset="utf-8">
  <title>GR00T LIBERO Interactive Report</title>
  <style>
...
<body>
<main>
  <h1>GR00T LIBERO Interactive Report</h1>
...
def write_html(
    *,
    ...
    rows = [
        ...
        f"<h1>GR00T LIBERO Report: task {task_id}, episode {episode}</h1>",
        f"<p><b>Run:</b> {html.escape(str(run_dir))}</p>",
        f"<p><b>Task:</b> {html.escape(trim_task(task, 220))}</p>",
        "<p>Each chunk row is one policy call. GR00T executes the first 8 predicted actions, then replans from fresh simulator observations.</p>",
        "<ul>",
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create Colab-visible granular reports for GR00T LIBERO activation captures.")
